...
def main():
    """
    Standalone entry point to run the dual GEMM kernel locally.
    Generates test data, runs the kernel, verifies correctness, and reports timing.
    """
    import argparse
    import time
    ...
    parser = argparse.ArgumentParser(description="Run NVFP4 Dual GEMM kernel standalone")
    ...
    parser.add_argument("--l", type=int, default=1, help="Batch size (L dimension)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--warmup", type=int, default=3, help="Number of warmup iterations")
    parser.add_argument("--iters", type=int, default=10, help="Number of timing iterations")
    parser.add_argument("--skip-verify", action="store_true", help="Skip verification against reference")
    args = parser.parse_args()

    print(f"Running NVFP4 Dual GEMM: M={args.m}, N={args.n}, K={args.k}, L={args.l}")
    print(f"Kernel computes: C = silu(A @ B1) * (A @ B2)")
    print()

    # Generate input data
    print("Generating input data...")
    data = generate_input(args.m, args.n, args.k, args.l, args.seed)
    print("Input data generated.")
    print()

    # Compile kernel (first call compiles and caches)
    ...
    avg_time_ms = (end - start) / args.iters * 1000
    print(f"Average kernel time: {avg_time_ms:.3f} ms")
    print()

    # Verify correctness
    if not args.skip_verify:
        print("Verifying against reference implementation...")
        # Run custom kernel one more time with fresh output
        data[-1].zero_()
        ...
        # Compare
        errors = verbose_allclose(output, expected, rtol=1e-03, atol=1e-03)
        if errors:
