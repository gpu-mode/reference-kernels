...
class SharedStorage:
    ...
    reduction_buf: cute.struct.MemRange[cutlass.Float32, mma_tiler_mnk[0] * mma_tiler_mnk[1]]
...
def kernel(
    ...
    # (TILE_M, TILE_N)
    sReduction = cute.make_tensor(
        storage.reduction_buf.data_ptr(),
    ...
    # (MMA, MMA_M, MMA_N)
    tCsReduction = thr_mma.partition_C(sReduction)
    ...
    tTR_sReduction = thr_copy_t2r.partition_D(tCsReduction)
    # (T2R_M, T2R_N, EPI_M, EPI_N）
    ...
    # Copy from register to shmem
    cute.autovec_copy(tTR_rAcc, tTR_sReduction)

    # wait for all CTAs cluster to store the partial sum
    cute.arch.cluster_arrive()
    cute.arch.cluster_wait()

    _, _, block_rank = cute.arch.block_in_cluster_idx()

    if block_rank == 0:
        # Read from peer's shared memory and add to own accumulator
        acc_ssa = tTR_rAcc.load()
        
    
    # wait for the first CTA in cluster to reduce
    cute.arch.cluster_arrive()
    cute.arch.cluster_wait()
    ...
    #     cute.printf("tiled_copy_t2r: {}", tiled_copy_t2r.layout_tv_tiled.shape)
    #     cute.printf("tTR_rAcc: {}", tTR_rAcc.shape)
    #     cute.printf("tTR_gC: {}", tTR_gC.shape)
    #     cute.printf("tTR_sReduction: {}", tTR_sReduction.shape)
