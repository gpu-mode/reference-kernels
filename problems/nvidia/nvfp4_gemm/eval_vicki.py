...
def run_benchmarking(
    ...
    run_single_benchmark(pool, tests[0], False, 2, 10e7)
    ...
    for idx, test in enumerate(tests):
        ...
        result = run_single_benchmark(pool, test, False, 2, 10e9)
    ...
    with PopcornOutput(int(fd)) as logger:
        ...
        with mp_context.Pool(1) as pool:
            ...
            if mode == "leaderboard":
                ...
                run_single_benchmark(pool, tests[0], False, 2, 1e7)
                
                # Step 3: Run leaderboard benchmarks (compilation time excluded)
                logger.log("benchmark-count", len(tests))
                passed = True
                for i in range(len(tests)):
                    result = run_single_benchmark(pool, tests[i], True, 2, 30e9)
