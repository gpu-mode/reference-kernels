...
def run_single_test(test: TestCase):
    """
    Runs a single test directly (no multiprocessing).
    """
...
def run_benchmarking(logger: PopcornOutput, tests: list[TestCase]):
    ...
    """
    # warm up
    run_single_benchmark(tests[0], False, 100, 10e7)

    passed = True
    logger.log("benchmark-count", len(tests))
    for idx, test in enumerate(tests):
        logger.log(f"benchmark.{idx}.spec", test.spec)
        result = run_single_benchmark(test, False, 100, 10e9)
...
def main():
    ...
    with PopcornOutput(int(fd)) as logger:
        ...
        if mode == "leaderboard":
            # warmup
            run_single_benchmark(tests[0], False, 100, 1e7)
            logger.log("benchmark-count", len(tests))
            passed = True
            for i in range(len(tests)):
                result = run_single_benchmark(tests[i], True, 100, 30e9)
