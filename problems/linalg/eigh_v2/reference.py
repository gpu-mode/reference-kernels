...
def _apply_case(a: torch.Tensor, case: str, cond: int, gen: torch.Generator) -> torch.Tensor:
    ...
    if case == "dense":
        ...
        profiles = ("dense", "spectrum", "psd", "repeated", "clustered")
        max_block = max(1, min(128, n // 2 if n > 1 else 1))
        for b in range(batch):
            ...
            while start < n:
                ...
                if remaining <= max_block:
                    ...
                    block = int(torch.randint(1, max_block + 1, (1,), device=device, generator=gen).item())
                profile = profiles[
                    int(torch.randint(0, len(profiles), (1,), device=device, generator=gen).item())
                ]
                block_a = a[b : b + 1, start : start + block, start : start + block]
                out[b : b + 1, start : start + block, start : start + block] = _apply_case(
                    block_a,
                    profile,
                    cond,
                    gen,
