...
from utils import checkAllclose, perftest
from src.kernels.triton.a8w8_gemm.blockwise_a8w8_gemm import w8a8_block_fp8_matmul
...
@perftest()
def run_gemm_triton(x, weight, x_scale, w_scale, dtype=torch.bfloat16):
    return w8a8_block_fp8_matmul(x, weight, x_scale, w_scale, block_shape, dtype)
...
def test_gemm(dtype, m, n, k):
    ...
    b, avg_b = run_gemm_triton(x, weight, x_scale, w_scale, dtype)

    msg = f"[perf] dim: {str(dim):<20} dtype: {dtype}, torch avg: {avg_a:<8.2f} us, triton avg: {avg_b:<8.2f} us, uplift: {avg_a/avg_b -1:<5.1%}"
